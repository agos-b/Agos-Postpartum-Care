...
async def ph_step4(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "5. Event Address / የዝግጅቱ አድራሻ:",
        reply_markup=get_nav_kb(lang, back_callback='ph_back')
    )
    return PH_ADDR

async def ph_step5(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Get event address and show package selection"""
    ...
    if 'ph_package' not in context.user_data:
        ...
        # Skip to payment step
        return await ph_step6(update, context)

async def ph_step6(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle package selection and show payment instructions"""
    # Handle both callback query and message cases
    if hasattr(update, 'callback_query') and update.callback_query:
        query = update.callback_query
        await query.answer()
...
async def ph_final(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    summary = (
        ...
        f"📱 Client Telegram: {context.user_data.get('ph_username')}"  # ADD THIS LINE
    )
...
if __name__ == '__main__':
    ...
    ph_conv = ConversationHandler(
        ...
        states={
            ...
            PH_USERNAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, ph_step3)],
            PH_DATE: [MessageHandler(filters.TEXT & ~filters.COMMAND, ph_step4)],
            PH_ADDR: [MessageHandler(filters.TEXT & ~filters.COMMAND, ph_step5)],
            PH_PACKAGE: [CallbackQueryHandler(ph_step6)],
